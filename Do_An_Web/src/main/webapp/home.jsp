       ...
       class="${category == cat ? 'active' : ''}">${cat}</a>
		...
		<c:if test="${empty products}">
			<div class="empty-note">
				<c:choose>
					<c:when test="${type == 'all' || empty type}">
						Chưa có sản phẩm nào trong hệ thống. Vui lòng thêm sản phẩm vào database.
					</c:when>
					<c:otherwise>
						Chưa có sản phẩm cho mục này. (Kiểm tra dữ liệu bảng Products và cột type = '${type}')
					</c:otherwise>
				</c:choose>
			</div>
		</c:if>
